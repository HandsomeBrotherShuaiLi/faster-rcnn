...
import random
import copy
from model import data_augment
from model import roi_helpers
import threading
import itertools

random.seed(0)
def get_out_length(input_length):
    #zero padding
    input_length+=6
    filter_size=[7,3,1,1]
    stride=2
    for filter in filter_size:
        input_length=(input_length-filter+stride)//stride
    return input_length
def get_img_output_length(width,height):
    return get_out_length(width),get_out_length(height)
def union(au,bu):
    x=min(au[0],bu[0])
    y=min(au[1],bu[1])
    w=max(au[2],bu[2])-x
...
def calc_rpn(C,img_data,width,height,resized_width,resized_height):
    ...
    num_anchors=len(anchor_sizes)*len(anchor_sizes)

    #calculate the output map size based n the network architecture
    (output_width,output_height)=get_out_length(resized_width,resized_height)
